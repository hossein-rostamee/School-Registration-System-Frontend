...
export default function Student(props) {
  ...
  const handleDeleteWarning = (e) => {
    ...
    if (e.target.id == "yes") {
      ...
      axios
        .get(
          `http://185.206.93.9:8000/register/liststu/delete/${props.student.id}/${location.state.username}/${location.state.password}/`
        )
        .then((res) => {
          if (res.data == "deleted") {
            axios
              .delete(
                `http://185.206.93.9:8000/register/deleteuser/${props.student.user}/${location.state.username}/${location.state.password}/`
              )
              .then((res) => {
                if (res.data == "deleted") {
                  setLoading(false);
